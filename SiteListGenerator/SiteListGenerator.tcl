...
foreach logFile $logFileList {
  ...
  set totalCounter 0
  set loadedSW ""
  set selFileNameFull [string trim [lindex [split $logFile "."] 0]]
...
foreach line [split $content "\n"] {
  ...
  if {$line != ""} {
    if {$selFileGenerate == "selGen"} {
	  ...
	  set firstUs [string first "_" $siteNoName]
      set firstUs [expr $firstUs - 1]   
      # siteNo: 83463 
	  set siteNo [string range $siteNoName 0 $firstUs]
	  set firstUs [expr $firstUs + 2]
	  # siteName: JurongWestBlk461
	  set siteName [string range $siteNoName $firstUs end]
